...
class Game {
    ...
    // rvo stuff
    public rvo_is_ini:boolean = false ;
    public goals: Vector2[] = [];
    public sim: Simulator;

    public na: number = 55;

    public hh: number = 860;
    public ww: number = 1100;
    ...
    private allAssetsLoaded(event): void {
        ...
        // add some sprites
          ...
          for (var j = 1 ; j < 4 ; j++) {
              for (var i: number = 0 ; i < this.na ; i++) {

                  var v: Vector2 = this.sim.getAgentPosition(i);
                   if(j == 1) var sprite: Sprites = new Sprites(AssetsManager.array_jpg[1].clone(), v.x, v.y);
                  if (j == 2) var sprite: Sprites = new Sprites(AssetsManager.array_jpg[2].clone(), v.x, v.y);
                  if (j == 3) var sprite: Sprites = new Sprites(AssetsManager.array_jpg[3].clone(), v.x, v.y);
                  if (j == 1) sprite.setScale(.7); 
    ...
    public setupScenario(sim: Simulator) {
        ...
        for (var j = 1 ; j < 4 ; j++) {
            for (var i = 0 ; i < n ; i++) {
                ...
                if (j == 1) { sim.addAgent(v.mul_k(200)); /*sim.setAgentMaxSpeed(c,1)*/}
                if (j == 2) { sim.addAgent(v.mul_k(330)); /*sim.setAgentMaxSpeed(c, 1)*/ }
                if (j == 3) { sim.addAgent(v.mul_k(410)); /*sim.setAgentMaxSpeed(c, 1) */}

                if (j == 1) sim.setAgentRadius(i, 12); 
	 ...
	 */
         ...
         var obstacle1: Vector2[] = [];
         var obstacle2: Vector2[] = [];
         var obstacle3: Vector2[] = [];
         var obstacle4: Vector2[] = [];
    ...
    public setPreferredVelocities(sim: Simulator) {
        ...
        for (var i = 0; i < sim.getNumAgents() ; i++) {
            var goalVector: Vector2 = this.goals[i].moins(sim.getAgentPosition(i));
